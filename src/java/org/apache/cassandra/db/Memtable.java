...
public class Memtable implements Comparable<Memtable> {
  ...
  private static MemtablePool createMemtableAllocatorPool() {
    ...
    case unslabbed_heap_buffers:
      return new HeapPool(heapLimit, DatabaseDescriptor.getMemtableCleanupThreshold(),
          new ColumnFamilyStore.FlushLargestColumnFamily());
    case heap_buffers:
      return new SlabPool(heapLimit, 0, DatabaseDescriptor.getMemtableCleanupThreshold(),
          new ColumnFamilyStore.FlushLargestColumnFamily());
    case offheap_buffers:
      if (!FileUtils.isCleanerAvailable) {
        throw new IllegalStateException(
            "Could not free direct byte buffer: offheap_buffers is not a safe "+
            "memtable_allocation_type without this ability, please adjust your config."+
            " This feature is only guaranteed to work on an Oracle JVM. Refusing to start.");
      }
      return new SlabPool(heapLimit, offHeapLimit, DatabaseDescriptor.getMemtableCleanupThreshold(),
          new ColumnFamilyStore.FlushLargestColumnFamily());
    case offheap_objects:
      return new NativePool(heapLimit, offHeapLimit, DatabaseDescriptor.getMemtableCleanupThreshold(),
          new ColumnFamilyStore.FlushLargestColumnFamily());
    default:
  ...
  private static final int ROW_OVERHEAD_HEAP_SIZE = estimateRowOverhead(
  ...
  // the precise lower bound of CommitLogPosition owned by this memtable; equal to
  // its predecessor's commitLogUpperBound
  private AtomicReference<CommitLogPosition> commitLogLowerBound;

  // The approximate lower bound by this memtable; must be <= commitLogLowerBound
  // once our predecessor
  ...
  // decide if this memtable should take the write, or if it should go to the next
  // memtable
  public boolean accepts(OpOrder.Group opGroup, CommitLogPosition commitLogPosition) {
    // if the barrier hasn't been set yet, then this memtable is still taking ALL
    // writes
    OpOrder.Barrier barrier = this.writeBarrier;
    if (barrier == null)
      return true;
    // if the barrier has been set, but is in the past, we are definitely destined
    // for a future memtable
  ...
  /**
   * @return true if this memtable is expired. Expiration time is determined by
   *         CF's memtable_flush_period_in_ms.
   */
  ...
  /**
   * Should only be called by ColumnFamilyStore.apply via Keyspace.apply, which
   * supplies the appropriate OpOrdering.
   *
   * commitLogSegmentPosition should only be null if this is a secondary index, in
  ...
  long put(PartitionUpdate update, UpdateTransaction indexer, OpOrder.Group opGroup) {
    ...
    if (previous == null) {
      ...
      if (previous == null) {
        previous = empty;
        // allocate the row overhead after the fact; this saves over allocating and
        // having to free after, but
  ...
  public String toString() {
    return String.format("Memtable-%s@%s(%s serialized bytes, %s ops, %.0f%%/%.0f%% of on/off-heap limit)", cfs.name,
        hashCode(), FBUtilities.prettyPrintMemory(liveDataSize.get()), currentOperations,
  ...
  public MemtableUnfilteredPartitionIterator makePartitionIterator(final ColumnFilter columnFilter,
    ...
    if (startIsMin)
      ...
      subMap = stopIsMin ? partitions.tailMap(keyRange.left, includeStart)
    ...
    return new MemtableUnfilteredPartitionIterator(cfs, iter, isForThrift, minLocalDeletionTime, columnFilter,
        dataRange);
  }
  ...
  /**
   * For testing only. Give this memtable too big a size to make it always fail
   * flushing.
   */
  @VisibleForTesting
  ...
  class FlushRunnable implements Callable<SSTableMultiWriter> {
    ...
    FlushRunnable(PartitionPosition from, PartitionPosition to, Directories.DataDirectory flushLocation,
    ...
    FlushRunnable(ConcurrentNavigableMap<PartitionPosition, AtomicBTreePartition> toFlush,
        Directories.DataDirectory flushLocation, PartitionPosition from, PartitionPosition to,
        LifecycleTransaction txn) {
    ...
    private void writeSortedContents() {
      ...
      logger.debug("Completed flushing {} ({}) for commitlog position {}", writer.getFilename(),
          FBUtilities.prettyPrintMemory(bytesFlushed), commitLogUpperBound);
      // Update the metrics
      cfs.metric.bytesFlushed.inc(bytesFlushed);

      if (heavilyContendedRowCount > 0)
        logger.trace("High update contention in {}/{} partitions of {} ", heavilyContendedRowCount, toFlush.size(),
            Memtable.this);
    }

    public SSTableMultiWriter createFlushWriter(LifecycleTransaction txn, String filename, PartitionColumns columns,
        EncodingStats stats) {
      MetadataCollector sstableMetadataCollector = new MetadataCollector(cfs.metadata.comparator)
          .commitLogIntervals(new IntervalSet<>(commitLogLowerBound.get(), commitLogUpperBound.get()));

      return cfs.createSSTableMultiWriter(Descriptor.fromFilename(filename), toFlush.size(),
          ActiveRepairService.UNREPAIRED_SSTABLE, sstableMetadataCollector,
  ...
  private static int estimateRowOverhead(final int count) {
    ...
    try (final OpOrder.Group group = new OpOrder().start()) {
      ...
      for (int i = 0; i < count; i++)
        partitions.put(
            allocator.clone(new BufferDecoratedKey(new LongToken(i), ByteBufferUtil.EMPTY_BYTE_BUFFER), group), val);
  ...
  public static class MemtableUnfilteredPartitionIterator extends AbstractUnfilteredPartitionIterator {
    ...
    public MemtableUnfilteredPartitionIterator(ColumnFamilyStore cfs,
        Iterator<Map.Entry<PartitionPosition, AtomicBTreePartition>> iter, boolean isForThrift,
        int minLocalDeletionTime, ColumnFilter columnFilter, DataRange dataRange) {
      this.cfs = cfs;
      this.iter = iter;
